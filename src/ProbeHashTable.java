...
public class ProbeHashTable<K,V> extends AbstractHashMap<K,V> {
	private MapEntry<K,V>[] table;
	private String hashType;
	public ProbeHashTable(String hash) {super(); hashType=hash;}
	public ProbeHashTable(float load,String hash) {super(load); hashType=hash;}
	public ProbeHashTable(int cap,float load, String hash) {super(cap,load); hashType=hash;}
	...
	protected boolean bucketGet(K key) throws IOException {
		int hash = hashFunction(key, hashType);//finding hash
	...
	protected void resize() throws IOException {
        ...
        for (int i = 0; i < oldTable.length; i++) {
        	if (oldTable[i] != null ) {
            	  int hash=hashFunction(oldTable[i].getKey(),hashType);
            	  if(table[hash]!=null) {
	...
	private int hashFunction(K key , String hashType) throws IOException {
		int hashVal =  hashcode((String)key , hashType) % table.length;
		if(hashVal<0)
